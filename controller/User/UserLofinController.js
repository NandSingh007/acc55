...
exports.registerfrontendData = async (req, res) => {
  const { name, phone, email, pancard, aadharcard, address } = req.body;
  console.log(name, phone, email, pancard, aadharcard, address);

  try {
    // Check if user with same email, pancard or aadharcard already exists
    const existingUser = await FrontRegistrationData.findOne({
      $or: [{ email }, { pancard }, { aadharcard }]
    });
    ...
    if (existingUser) {
      ...
      } else if (existingUser.pancard === pancard) {
        duplicateField = "Pancard";
